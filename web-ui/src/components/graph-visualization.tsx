...
export function GraphVisualization({ app }: { app: string }) {
  ...
  useEffect(() => {
    const token = localStorage.getItem('auth-token');
    const es = new EventSource(`http://localhost:8081/api/graph/stream/${app}?token=${token}`);
  ...
  useEffect(() => {
    ...
    const flowNodes: Node[] = graph.nodes.map((n, index) => {
      const colors = getNodeColor(n.type, n.status);
      return {
        id: n.id,
        type: 'default',
        data: { label: `${n.name}\n(${n.status})` },
        position: { x: (index % 4) * 250, y: Math.floor(index / 4) * 150 },
        className: `${colors} text-white px-4 py-2 rounded-lg border-2 shadow-md`,
        style: {
          borderRadius: '12px',
          padding: '10px 16px',
          minWidth: '120px',
