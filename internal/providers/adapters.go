...
// Helper function to convert database ResourceInstance to SDK Resource
func databaseResourceToSDKResource(dbResource *database.ResourceInstance) *sdk.Resource {
	// Convert database hints to SDK hints
	hints := make([]sdk.Hint, len(dbResource.Hints))
	for i, dbHint := range dbResource.Hints {
		hints[i] = sdk.Hint{
	...
	// Parse configuration - it's already map[string]interface{} in database model
	var config sdk.Config
	if dbResource.Configuration != nil {
		config = sdk.NewMapConfig(dbResource.Configuration)
	} else {
		config = sdk.NewMapConfig(make(map[string]interface{}))
	}

	// Convert pointer fields to strings
	providerID := ""
	if dbResource.ProviderID != nil {
		providerID = *dbResource.ProviderID
	}

	errorMessage := ""
	if dbResource.ErrorMessage != nil {
		errorMessage = *dbResource.ErrorMessage
	}

	return &sdk.Resource{
		ID:              dbResource.ID,
		ApplicationName: dbResource.ApplicationName,
		ResourceName:    dbResource.ResourceName,
		ResourceType:    dbResource.ResourceType,
		State:           sdk.ResourceState(dbResource.State),
		HealthStatus:    dbResource.HealthStatus,
