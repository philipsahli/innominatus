...
func (i *Installer) VerifyKubeContext() error {
	...
	cmd := exec.Command("kubectl", "config", "get-contexts", i.kubeContext)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("Kubernetes context '%s' not found. Make sure Docker Desktop is running", i.kubeContext)
	}

	// Test connectivity
	cmd = exec.Command("kubectl", "--context", i.kubeContext, "cluster-info")
	if err := cmd.Run(); err != nil {
...
func (i *Installer) CreateNamespace(namespace string) error {
	...
	cmd := exec.Command("kubectl", "--context", i.kubeContext, "create", "namespace", namespace)
...
func (i *Installer) InstallComponent(component DemoComponent) error {
	...
	cmd := exec.CommandContext(installCtx, "helm", "upgrade", "--install", component.Name,
		chartRef,
		"--version", component.Version,
		"--namespace", component.Namespace,
		"--kube-context", i.kubeContext,
	...
	for {
		...
		case <-statusTicker.C:
			...
			// Show pod status
			statusCmd := exec.Command("kubectl", "get", "pods", "-n", component.Namespace, "--no-headers")
...
func (i *Installer) UninstallComponent(component DemoComponent) error {
	...
	cmd := exec.Command("helm", "uninstall", component.Name,
		"--namespace", component.Namespace,
		"--kube-context", i.kubeContext)
...
func (i *Installer) DeleteNamespace(namespace string) error {
	...
	cmd := exec.Command("kubectl", "--context", i.kubeContext, "delete", "namespace", namespace, "--ignore-not-found=true")
	if err := cmd.Run(); err != nil {
...
func (i *Installer) WaitForDeployment(namespace, deploymentName string, timeout time.Duration) error {
	...
	cmd := exec.Command("kubectl", "--context", i.kubeContext, "wait",
...
func (i *Installer) CheckHelmRelease(releaseName, namespace string) (bool, error) {
	cmd := exec.Command("helm", "list", "-n", namespace, "--kube-context", i.kubeContext, "-q")
...
func (i *Installer) InstallKubernetesDashboard() error {
	...
	cmd := exec.Command("kubectl", "--context", i.kubeContext, "apply", "-f", dashboardURL)
	output, err := cmd.CombinedOutput()
	if err != nil {
